import 'dart:convert';
import 'dart:io';
import 'package:event_country/mutations.dart';
...
import 'package:event_country/mutations.dart' as mutuations;
...
class _createEventState extends State<createEvent> {
  ...
  cargarPref() async {
   ...
   Future<String> addImage(image ) async {
          ...
          return queryResult.data['uploadPhotoEvent']['message'].toString();
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (eventData != null) {
      ...
      setState(() {
        ...
        if (eventModel.publicType == "public")
          privacity = false;
        else
          privacity = true;
      });
  ...
  _mostrarFoto() {
    if (fotoUrl != null) {
      ...
      if (foto != null) {
        //final bytes = File(foto).readAsBytesSync();
        //   List<int> imageBytes = foto.readAsBytesSync();
        //  iamge = base64Encode(imageBytes);
        File imageFile = new File(foto.path);
        List<int> imageBytes = imageFile.readAsBytesSync();
        iamge = base64Encode(imageBytes);
        addImage(iamge).then((resp){
          eventModel.image = resp;
        });
        //eventModel.image =  addImage(iamge);
        return Image.file(
          foto,
          fit: BoxFit.cover,
          height: 300.0,
  ...
  void _submit() async {
    ...
    // final img = await addImage(iamge);
    // eventModel.image = img;
    ...
    if (eventModel.id == null) {
      ...
      if(foto ==null){
        _mostrarAlert(context, 'Error!', 'Limagen es necesaria', 'interfaz', 'no');
      }
      else{
         
      if (eventModel.publicType == null) eventModel.publicType = "public";
        print('evento: ${eventModel.publicType}');
  ...
  void _mostrarAlert(BuildContext context, String titulo, String text,
      String img, String back) {
    showDialog(
        context: context,
  ...
  _createCapacidad() {
    return TextFormField(
      initialValue: (eventModel.capacity) == 0 ? '': eventModel.capacity.toString() ,
      keyboardType: TextInputType.numberWithOptions(decimal: true),
      style: TextStyle(
          fontFamily: "WorkSofiaSemiBold", fontSize: 18.0, color: Colors.black),
      decoration: InputDecoration(
      ...
      children: <Widget>[
        ...
        Expanded(
          child: SwitchListTile(
              ...
              title: Text(
                'Evento Privado',
                style: TextStyle(fontFamily: "Sofia", fontSize: 15.0),
              ),
              activeColor: Colors.deepPurple,
              onChanged: (bool value) {
                //print(value);
                value
                    ? eventModel.publicType = "private"
                    : eventModel.publicType = "public";
                setState(() {
                  privacity = value;
                });
              }),
